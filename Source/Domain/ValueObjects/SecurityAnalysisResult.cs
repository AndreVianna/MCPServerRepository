...
public record SecurityAnalysisResult
{
    ...
    {
        if (string.IsNullOrWhiteSpace(analyzerName)) throw new ArgumentException("Analyzer name cannot be null or empty", nameof(analyzerName));
        if (string.IsNullOrWhiteSpace(analyzerVersion)) throw new ArgumentException("Analyzer version cannot be null or empty", nameof(analyzerVersion));
