...
public class SecurityPolicy : BaseEntity
{
    ...
    {
        if (string.IsNullOrWhiteSpace(name)) throw new ArgumentException("Policy name cannot be null or empty", nameof(name));
        if (string.IsNullOrWhiteSpace(description)) throw new ArgumentException("Policy description cannot be null or empty", nameof(description));
