...
/// </summary>
public class CachedMcpHubApiClient : IMcpHubApiClient {
    private readonly ILogger<CachedMcpHubApiClient> _logger;
    private readonly IMcpHubApiClient _innerClient;
    ...
    public async Task<PackageListResponse> GetPackagesAsync(int page = 1, int pageSize = 20, CancellationToken cancellationToken = default) {
        ...
        return await _offlineMode.ExecuteWithFallbackAsync(
            onlineOperation: async ct => {
                return await _innerClient.GetPackagesAsync(page, pageSize, ct).ConfigureAwait(false);
            },
            offlineOperation: async ct => {
    ...
    public async Task<bool> TestConnectivityAsync(CancellationToken cancellationToken = default) {
        ...
        return await _innerClient.TestConnectivityAsync(cancellationToken).ConfigureAwait(false);
    }
