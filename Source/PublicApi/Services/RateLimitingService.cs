...
public class RateLimitingService : IRateLimitingService, IDisposable {
    ...
    private static RateLimitStrategy ParseStrategy(string strategy) => strategy.ToLowerInvariant() switch {
    ...
    private static string GetCacheKey(string identifier, string policy) => $"rate_limit:{policy}:{identifier}";
