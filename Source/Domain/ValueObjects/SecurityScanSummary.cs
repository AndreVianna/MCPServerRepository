...
public record SecurityScanSummary {
    ...
    public SecurityScanSummary(
        ...
        if (criticalIssues < 0)
            throw new ArgumentException("Critical issues count cannot be negative", nameof(criticalIssues));
        if (highIssues < 0)
            throw new ArgumentException("High issues count cannot be negative", nameof(highIssues));
        if (mediumIssues < 0)
            throw new ArgumentException("Medium issues count cannot be negative", nameof(mediumIssues));
        if (lowIssues < 0)
            throw new ArgumentException("Low issues count cannot be negative", nameof(lowIssues));
        if (string.IsNullOrWhiteSpace(securityGrade))
            throw new ArgumentException("Security grade cannot be null or empty", nameof(securityGrade));
