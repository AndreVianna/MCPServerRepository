...
public abstract class CachedRepository<TEntity>(IRepository<TEntity> repository, ICacheService cacheService, string cacheKeyPrefix) : IRepository<TEntity> where TEntity : class {
    ...
    protected readonly string _cacheKeyPrefix = cacheKeyPrefix;
    ...
    public virtual async Task<TEntity?> FirstOrDefaultAsync(Expression<Func<TEntity, bool>> predicate, CancellationToken cancellationToken = default) => await _repository.FirstOrDefaultAsync(predicate, cancellationToken);

    public virtual async Task<bool> AnyAsync(Expression<Func<TEntity, bool>> predicate, CancellationToken cancellationToken = default) => await _repository.AnyAsync(predicate, cancellationToken);

    public virtual async Task<int> CountAsync(Expression<Func<TEntity, bool>>? predicate = null, CancellationToken cancellationToken = default) => await _repository.CountAsync(predicate, cancellationToken);
    ...
    protected virtual async Task InvalidateCacheAsync(CancellationToken cancellationToken = default) => await _cacheService.RemovePatternAsync($"{_cacheKeyPrefix}:*", cancellationToken);
}