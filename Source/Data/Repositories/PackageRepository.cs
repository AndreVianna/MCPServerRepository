...
public class PackageRepository(McpHubContext context) : Repository<Package>(context), IPackageRepository {
    ...
    public async Task<List<Package>> GetByPublisherIdAsync(Guid publisherId, CancellationToken cancellationToken = default) => await _dbSet
            .Include(p => p.Publisher)
            .Where(p => p.PublisherId == publisherId)
            .OrderByDescending(p => p.CreatedAt)
            .ToListAsync(cancellationToken);

    public async Task<List<Package>> GetByStatusAsync(PackageStatus status, CancellationToken cancellationToken = default) => await _dbSet
            .Include(p => p.Publisher)
            .Where(p => p.Status == status)
            .OrderByDescending(p => p.CreatedAt)
            .ToListAsync(cancellationToken);

    public async Task<List<Package>> GetByTrustTierAsync(TrustTier trustTier, CancellationToken cancellationToken = default) => await _dbSet
            .Include(p => p.Publisher)
    ...
    public async Task<List<Package>> GetByTagsAsync(List<string> tags, CancellationToken cancellationToken = default) => await _dbSet
            .Include(p => p.Publisher)
            .Where(p => tags.Any(tag => p.Tags.Contains(tag)))
            .OrderByDescending(p => p.CreatedAt)
            .ToListAsync(cancellationToken);

    public async Task<List<Package>> GetRecentlyUpdatedAsync(int count = 10, CancellationToken cancellationToken = default) => await _dbSet
            .Include(p => p.Publisher)
